...
final class EnumEncoder: AbstractEncodingNode, SingleValueEncodingContainer {
	...
	func encode<Element>(_ value: [Element]) throws where Element: EncodableContainer {
		self.encodedValue = value
	}
	
	func encode<Key, Value>(_ value: [Key: Value]) throws where Key: ProtobufMapKey, Value: Encodable {
		self.encodedValue = value
	}
